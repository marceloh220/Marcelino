...
void setup() {
  ...
  timer.prescale(P_128);                //Prescaler of timer2 configured with prescaler 1/128
  timer.attach(OVF, ASYNCHRON, blink);  //Function blink attached in overflow interrupt of timer2 in asynchronous mode with external crystal

  pwm.mode(FAST);                       //PWM in fast mode
  pwm.pwmA(50);                         //PWM with 50 of 255 width
  //PWM frequency
  // Fpwm = F / ( prescale * 2^8 )
  // Fpwm = 8e6 / ( 64 * 256 ) = 488.28Hz
  // Only the timer 2 is working in 32.768Hz frequency
